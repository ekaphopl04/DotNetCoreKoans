...
namespace DotNetCoreKoans.Koans
{
    public class AboutTuples : Koan
    {
        ...
        public void TupleCanContainDifferentTypes()
        {
            ...
            Assert.Equal(typeof(System.Collections.Generic.List<string>), batman1966.enemy.GetType());
        ...
        public void TupleReplaceOutParameter()
        {
            ...
            Assert.Equal(string.Join(",", otherEnemies), string.Join(",", otherEnemies));

            var extract = extractMainEnemyWithTuple("Joker,Penguin,Riddler,Catwoman");

            Assert.Equal("Joker", extract.mainEnemy);
            Assert.Equal(string.Join(",", extract.othersEnemies), string.Join(",", extract.othersEnemies));
